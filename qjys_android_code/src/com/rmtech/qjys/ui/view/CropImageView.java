...
public class CropImageView extends ImageView implements View.OnTouchListener,
	...
	private void configPosition() {
		...
		minScale = Math.max((float) borderlength / drawableHeight,
				(float) borderlength / drawableWidth);
	...
	public final float getScale() {
		...
		return curDrawableHeight / getDrawable().getIntrinsicHeight();
	}
	...
	private void checkMatrixBounds() {
		...
		// 回弹动画
		TranslateAnimation trans = new TranslateAnimation(-deltaX, 0, -deltaY,
				0);
	...
	public Bitmap crop(float outputX, float outputY) {
		...
		// 获取截取位置的左上角相对坐标
		float cropDrawableTop = (viewHeight - borderlength) / 2;
		float cropDrawableLeft = (viewWidth - borderlength) / 2;
		...
		Bitmap bitmap;
		try {
			bitmap = Bitmap.createBitmap(d.getBitmap(),
					(int) (relativeLeft / curScale),
					(int) (relativeTop / curScale),
					(int) (borderlength / curScale),
					(int) (borderlength / curScale), m, true);
		} catch (Exception e) {
			return null;
		}
