...
//            L.w(TAG, "unable recycle bitmap", t);
        }
    }
...
//            L.e(TAG, "cre ateVideoThumbnail", e);
        } catch (InvocationTargetException e) {
//            L.e(TAG, "createVideoThumbnail", e);
    ...
    public  static Bitmap resizeFromTop(Bitmap bitmap, int width, int height) {
		...
		if(Math.abs(dstRatio - srcRatio) < 0.01f) {
			...
			if(width >= bitmap.getWidth() || height >= bitmap.getHeight()) {
				tempBitmap = resizeBitmapByScale(bitmap, Math.max((float)width / bitmap.getWidth(), (float) height / bitmap.getHeight()), false);
			} else {
				tempBitmap = resizeBitmapByScale(bitmap, Math.max((float)width / bitmap.getWidth(), (float) height / bitmap.getHeight()), false);
			}
			if(tempBitmap == null)
