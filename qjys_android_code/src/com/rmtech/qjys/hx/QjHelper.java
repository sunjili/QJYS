...
public class QjHelper {
	...
	private static QjHelper instance = null;

	private QjModel demoModel = null;
	...
	public synchronized static QjHelper getInstance() {
		if (instance == null) {
			instance = new QjHelper();
		}
		return instance;
	}
			...
			public void onConnected() {
				...
				if (isGroupsSyncedWithServer && isContactsSyncedWithServer) {
					new Thread() {
						@Override
						public void run() {
							QjHelper.getInstance().notifyForRecevingEvents();
						}
					}.start();
				} else {
	...
	private void onConteactAgreed(String username) {
...
//		EMClient.getInstance().chatManager().sendMessage(message);
	...
	public class MyContactListener implements EMContactListener {
		...
		public void onContactDeleted(String username) {
			// 被删除
//			Map<String, EaseUser> localUsers = QjHelper.getInstance().getContactList();
...
//			EventBus.getDefault().post(new DoctorEvent(DoctorEvent.TYPE_DELETE));
		...
		public void onContactInvited(String username, String reason) {
			...
			getNotifier().sendNotification(username, reason);
			notifyNewIviteMessage(msg);
			broadcastManager.sendBroadcast(new Intent(QjConstant.ACTION_CONTACT_CHANAGED));
		}
	...
	public QjModel getModel() {
		return (QjModel) demoModel;
	}
